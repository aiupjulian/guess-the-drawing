...
io.on('connection', socket => {
  const emitUsers = () =>
    io.to(socket.room).emit('users', getUsers(socket.room));
  ...
  socket.on('start game', () => {
    ...
    const emitPlays = async () => {
      ...
      if (!io.sockets.adapter.rooms[socket.room].play.word) {
        io.sockets.adapter.rooms[socket.room].play.word =
          io.sockets.adapter.rooms[socket.room].play.words[0]; // eslint-disable-line
      }
      io.to(socket.room).emit(
        'word chosen',
        io.sockets.adapter.rooms[socket.room].play
