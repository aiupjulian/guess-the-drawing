...
class Canvas extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      offsetHeight !== prevProps.offsetHeight ||
      offsetWidth !== prevProps.offsetWidth
    ) {
      this.onResize();
    }
  }
  ...
  getMouseHorizontalPositionInCanvas = mouseX =>
    mouseX - this.canvasHorizontalDistanceToWindows;

  getMouseVerticalPositionInCanvas = mouseY =>
    mouseY - this.canvasVerticalDistanceToWindows;
  ...
  handleUndoCanvas = emit => {
    ...
    this.drawnLinesArray.forEach(drawnLine => {
      this.drawLine(...drawnLine, true);
    });
    this.drawnLineId -= 1;
    if (!emit) {
      return;
  ...
  render() {
    ...
    return (
      <Fragment>
        ...
        <div
          className={`${css.options} ${disabled ? css.optionsDisabled : ''}`}
        >
          <Options
